...
const CENTRAL_AFRICA_COUNTRIES = [
  'DR Congo', 'Cameroon', 'Central African Republic', 'Chad', 
  'Gabon', 'Equatorial Guinea', 'Republic of Congo', 'São Tomé and Príncipe'
]

export default function CentralAfricaPage() {
  ...
  const loadArticles = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('articles')
        .select('*')
        .eq('category', 'Central Africa')
        .eq('status', 'published')
        .in('country', CENTRAL_AFRICA_COUNTRIES)
                ...
                {selectedArticle.content ? (
                  ...
                  <div className="text-gray-600 leading-relaxed">
                    <p className="mb-4">
                      ...
                      bringing you the most current news and insights from Central Africa.
                    </p>
                    <p className="mb-4">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 md:p-12">
            <div className="text-center">
              ...
              <p className="text-lg text-gray-600 mb-8 max-w-2xl mx-auto">
                We&apos;re working on bringing you comprehensive news coverage from Central Africa
              </p>
              
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm text-gray-500 max-w-2xl mx-auto">
                {CENTRAL_AFRICA_COUNTRIES.map((country, index) => (
                  <div key={index} className="bg-gray-50 rounded-lg p-3">
                    <div className="font-semibold text-gray-700">
                      {country === 'Central African Republic' ? 'CAR' : country}
                    </div>
                  </div>
                ))}
              </div>
