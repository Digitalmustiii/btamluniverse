...
// app/admin/security/edit/[id]/page.tsx
...
export default function EditSecurityPage() {
  ...
  const handleSubmit = async (status: 'draft' | 'published') => {
    ...
    try {
      ...
      alert(`Security article ${status === 'published' ? 'updated and published' : 'updated as draft'} successfully!`)
      router.push('/admin/security')
    } catch (error) {
      console.error('Error updating security article:', error)
        ...
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Link href="/admin/security" className="flex items-center gap-2 text-gray-600 hover:text-[#0F4007] transition-colors">
              ...
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-2xl font-bold text-gray-900">Edit Security Update</h1>
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200">
          <div className="p-8">
            <form className="space-y-8">
              {/* Title */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Security Update Title *</label>
                <input
                  type="text"
              ...
              {/* Thumbnail Upload */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Security Update Thumbnail *</label>
                {thumbnailPreview ? (
                  <div className="relative">
                    <div className="relative w-full h-64 rounded-lg overflow-hidden border-2 border-gray-200">
                      <Image src={thumbnailPreview} alt="Thumbnail preview" fill className="object-cover" sizes="100vw" />
                      <div className="absolute inset-0 bg-black bg-opacity-0 hover:bg-opacity-30 transition-all duration-200 flex items-center justify-center">
                        <button type="button" onClick={clearThumbnail} className="opacity-0 hover:opacity-100 bg-red-500 text-white p-2 rounded-full transition-opacity duration-200">
                          <X className="w-5 h-5" />
                        </button>
                      </div>
                    </div>
                  ...
                  <div className="relative">
                    <input type="file" id="thumbnail" accept="image/*" onChange={handleThumbnailUpload} disabled={uploadingImage} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
              ...
              {/* Category Selection */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Security Category *</label>
                    ...
                    return (
                      <div
                        ...
                        className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${
                          formData.category === category.value ? 'border-[#0F4007] bg-[#0F4007]/5' : 'border-gray-200 hover:border-gray-300'
                        }`}
                      >
                        <div className="flex items-center gap-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Country Focus *</label>
                  <select
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Threat Level</label>
                  <select
              ...
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  ...
                  />
                  <button type="button" onClick={handleAddTag} className="px-4 py-2 bg-[#0F4007] text-white rounded-lg hover:bg-[#0d3506] transition-colors">
                    Add
                  </button>
                </div>
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  Security Update Excerpt <span className="text-sm font-normal text-gray-500">(Optional - will be auto-generated if empty)</span>
                </label>
                <textarea
              ...
              {/* Content */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Security Update Content *</label>
              ...
              <div className="flex items-center justify-between pt-6 border-t border-gray-200">
                <div className="flex items-center gap-4">
                  <Link href="/admin/security" className="px-6 py-3 text-gray-600 hover:text-gray-800 font-medium transition-colors">
                    Cancel
                  </Link>
                  <button type="button" onClick={handleDelete} disabled={loading} className="flex items-center gap-2 px-6 py-3 bg-red-600 hover:bg-red-700 text-white rounded-lg font-medium transition-colors disabled:opacity-50">
                ...
                <div className="flex items-center gap-4">
                  <button type="button" onClick={() => handleSubmit('draft')} disabled={loading} className="flex items-center gap-2 px-6 py-3 bg-gray-600 hover:bg-gray-700 text-white rounded-lg font-medium transition-colors disabled:opacity-50">
                    <FileText className="w-4 h-4" />
                    {loading ? 'Saving...' : 'Save Draft'}
                  </button>
                  
                  <button type="button" onClick={() => handleSubmit('published')} disabled={loading} className="flex items-center gap-2 px-6 py-3 bg-[#0F4007] hover:bg-[#0d3506] text-white rounded-lg font-medium transition-colors disabled:opacity-50">
